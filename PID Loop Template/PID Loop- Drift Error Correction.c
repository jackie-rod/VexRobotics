...
{
	int ticksPerTurn = 3000; //experimental value
...
void PIDforward(float target, float waitTime, float maxPower = 1)
{
	...
	//FINAL
		finalPower = proportion + integral + derivative;
			...
			//ERROR CORRECTION
			error_drift = nMotorEncoder[RIGHTen] - nMotorEncoder[LEFTen];
			proportion_drift = Kp_C * error_drift;
			
			left(finalpower + proportion_drift);
			right(finalpower - proportion_drift);
		wait1Msec(40);
...
void PIDturn(float target, float waitTime, float maxPower = 1)
{
	...
	while(time1(T1) < betterTime(waitTime))
	{
		error = degreesToTicks (target) - (nMotorEncoder[LEFTen] - nMotorEncoder[RIGHTen]);
		proportion = Kp * error;
